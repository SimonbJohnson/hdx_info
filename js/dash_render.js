...
function createHeadLineFigure(id,bite,title){
    ...
    let number = String(parseInt(bite.bite.split('<span class="hbvalue">')[1].replace(/[^0-9\.]/g, ''))).replace(/(<([^>]+)>)/ig,"").replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    let headlineHTML = '<div class=" col-md-3"><div id="'+id.slice(1)+'text" class="headlinetext">'+title+'</div><div id="'+id.slice(1)+'number" class="headlinenumber">'+number+'</div></div>';
    $(id).append(headlineHTML);
}
...
function createChart(id,bite,sort,title){
    ...
    $(id).append('<div class="titlecontainer"><p class="bitetitletext">'+title+'</p><p class="bitetitle"></p></div><div id="chartcontainer'+id.substring(1)+'" class="chartelement"></div>');
    id = id.substring(1);
    $('#chartcontainer'+id).height(400);
...
function createMap(id,bite,data,mapOptions,title){
    ...
    $('#'+id).append('<div class="titlecontainer"><p class="bitetitletext">'+title+'</p><p class="bitetitle"></p></div><div id="'+id+'map" class="map"></div>');
